...
WRITE_ENABLED = os.getenv("ZENDESK_WRITE_ENABLED", "false").strip().lower() in {"1", "true", "yes", "on"}
...
def _require_write_enabled() -> None:
    if not WRITE_ENABLED:
        raise RuntimeError(
            "Write operations are disabled. Set ZENDESK_WRITE_ENABLED=true to enable them."
        )
...
def _zd_request(
    ...
    *,
    ...
    try:
        ...
        body = body[:1000]
        raise RuntimeError(f"Zendesk API error ({status_code}) for {path}: {body}") from exc
    except requests.RequestException as exc:
        raise RuntimeError(f"Zendesk request failed for {path}: {exc}") from exc
    ...
    try:
        ...
        raise RuntimeError(f"Zendesk returned a non-JSON response for {path}.") from exc
...
@mcp.tool()
def send_public_reply(ticket_id: int, reply: str, confirm: bool = False) -> Dict[str, Any]:
    _require_write_enabled()
    if not confirm:
        raise RuntimeError("Public reply not sent. Re-run with confirm=true to send.")
    _zd_request(
        "PUT",
        f"/tickets/{ticket_id}.json",
...
def create_ticket(
    ...
    return MutationResponse(success=True, ticket_id=data["ticket"]["id"], message="Ticket created.").model_dump()
...
def suggest_next_action(ticket_id: int) -> Dict[str, Any]:
    ...
    return NextActionResponse(ticket_id=ticket_id, recommendation=recommendation, rationale=rationale).model_dump()
